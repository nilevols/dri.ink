...
- `request` - 在Spring Web上下文中，单个bean的定义的范围限定为单个http请求的生命周期。
- `session` - 在Spring Web上下文中，单个bean的定义的范围限定为单个http会话的生命周期。
- `application` - 在Spring Web上下文中，单个bean的定义的范围限定为一个`ServletContext`的生命周期。
- `websocket` - 在Spring Web上下文中，单个bean的定义的范围限定为一个`WebSocket`的生命周期。
